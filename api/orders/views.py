...
order_model=order_namespace.model(
    'Order',{
        'id':fields.Integer(descrption="An ID"),
        'size':fields.String(desription="Size for order", required=True,
...
class GetUpateDelete(Resource):

    @order_namespace.marshal_with(order_model)
    @order_namespace.doc(
        description="Retrieve an order by it's ID",
        params={
    ...
    @order_namespace.marshal_with(order_model)
    @order_namespace.doc(
        desription="Update an order given an ID",
        params={
            "order_id":"An ID for a given order"
        }
    )
    ...
    @order_namespace.marshal_with(order_model)
    @order_namespace.doc(
        desription="Delete an Order given an order ID",
        params={
            "order_id":"An ID for a given order"
        }
    )
...
class UserOrder(Resource):
    ...
    @order_namespace.marshal_with(order_model)
    @order_namespace.doc(
        description="Get orders of a user give the user ID",
        params={
            "user_id":"A user's ID"
        }
    )
...
class UpdateOrderStatus(Resource):
    ...
    @order_namespace.marshal_with(order_model)
    @order_namespace.doc(
        desription="Update an order status given the ID",
        params={
            "order_id":"An ID for a given order"
        }
    )
